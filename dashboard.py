...
class ResultManagementSystem:
    def __init__(self, home):
        ...
        self.logo = self.logo.resize((90, 35), Image.LANCZOS)
        self.logo = ImageTk.PhotoImage(self.logo)

        # Title of project
        title = Label(self.home, text="Student Result Management", padx=10, compound=LEFT, image=self.logo,
                      font=("times new roman", 20, "bold"), bg="Navy Blue", fg="white")
        title.place(x=0, y=0, relwidth=1, height=50)

        # Menu
        Frame = LabelFrame(self.home, text="Menu", font=("times new roman", 15, "bold"), bg="white")
        Frame.place(x=10, y=70, width=1500, height=80)

        # SubMenu
        button_Course = Button(Frame, text="Course", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                               cursor="hand2", command=self.add_course)
        button_Course.place(x=20, y=5, width=200, height=40)

        button_Student = Button(Frame, text="Student", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                                cursor="hand2", command=self.add_student)
        button_Student.place(x=240, y=5, width=200, height=40)

        button_Result = Button(Frame, text="Result", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                               cursor="hand2", command=self.add_result)
        button_Result.place(x=460, y=5, width=200, height=40)

        button_View = Button(Frame, text="View Student Result", font=("ties new roman", 15, "bold"), bg="blue",
                             fg="white", cursor="hand2", command=self.add_view)
        button_View.place(x=685, y=5, width=200, height=40)

        button_Logout = Button(Frame, text="Logout", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                               cursor="hand2", command=self.logout)
        button_Logout.place(x=1120, y=5, width=200, height=40)

        button_Graph = Button(Frame, text="Student Graph View", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                              cursor="hand2", command=self.add_graph)
        button_Graph.place(x=900, y=5, width=200, height=40)

        button_Exit = Button(Frame, text="Exit", font=("ties new roman", 15, "bold"), bg="blue", fg="white",
                             cursor="hand2", command=self.exit)
        button_Exit.place(x=1355, y=5, width=100, height=40)

        # Content Window
        self.bgImage = Image.open("Images/7.jpg")
        self.bgImage = self.bgImage.resize((900, 350), Image.LANCZOS)
        self.bgImage = ImageTk.PhotoImage(self.bgImage)

        self.lbl_bg = Label(self.home, image=self.bgImage)
        self.lbl_bg.place(x=400, y=180, width=940, height=350)

        # Update Details
        self.totalCourse = Label(self.home, text="Total Courses \n 0 ", font=("times new roman", 20), bd=10,
                                 relief=RIDGE, bg="purple", fg="white")
        self.totalCourse.place(x=400, y=530, width=300, height=80)
        self.totalstudent = Label(self.home, text="Total Student \n 0 ", font=("times new roman", 20), bd=10,
                                  relief=RIDGE, bg="orange", fg="white")
        self.totalstudent.place(x=720, y=530, width=300, height=80)

        self.totalresults = Label(self.home, text="Total Results \n 0 ", font=("times new roman", 20), bd=10,
                                  relief=RIDGE, bg="coral", fg="white")
        self.totalresults.place(x=1040, y=530, width=300, height=80)

        # Footer
        footer = Label(self.home, text="Contact:9857642315", font=("times new roman", 13, "bold"), bg="grey",
    ...
    def add_graph(self):
        self.window2 = Toplevel(self.home)
        self.obj1 = GraphClass(self.window2)
